...
def format_record(idx, record):
    ...
    return f'  > id: {idx} ; lat = {lat} ; lon = {lon} ; lat_min = {lat_min} ; lat_max = {lat_max} ; ' + \
...
def prediction_analysis(file_prefix, channels_array, recorded_cyclones,
    ...
    false_positives = chunk_list_df[(chunk_list_df.pred_cat == common.CYCLONE_LABEL) &
                                    (chunk_list_df.true_cat == common.NO_CYCLONE_LABEL)]
    len_false_positives = len(false_positives)
    print(f'  > model has {len_false_positives} false positives')

    false_negatives = chunk_list_df[(chunk_list_df.pred_cat == common.NO_CYCLONE_LABEL) &
                                    (chunk_list_df.true_cat == common.CYCLONE_LABEL)]
    len_false_negatives = len(false_negatives)
    print(f'  > model has {len_false_negatives} false negatives')

    true_positives = chunk_list_df[(chunk_list_df.pred_cat == common.CYCLONE_LABEL) &
                                   (chunk_list_df.true_cat == common.CYCLONE_LABEL)]
    len_true_positives = len(true_positives)
    print(f'  > model has {len_true_positives} true positives')

    true_negatives = chunk_list_df[(chunk_list_df.pred_cat == common.NO_CYCLONE_LABEL) &
                                   (chunk_list_df.true_cat == common.NO_CYCLONE_LABEL)]
    ...
    print(classification_report(y_true=chunk_list_df.true_cat, y_pred=y_pred_cat_npy,
                                target_names=['no_cyclones', 'cyclones']))

    precision_cyclone    = precision_score(y_true=chunk_list_df.true_cat, y_pred=y_pred_cat_npy,
                                           pos_label=common.CYCLONE_LABEL)
    precision_no_cyclone = precision_score(y_true=chunk_list_df.true_cat, y_pred=y_pred_cat_npy,
                                           pos_label=common.NO_CYCLONE_LABEL)
    recall_cyclone    = recall_score(y_true=chunk_list_df.true_cat, y_pred=y_pred_cat_npy,
                                     pos_label=common.CYCLONE_LABEL)
    recall_no_cyclone = recall_score(y_true=chunk_list_df.true_cat, y_pred=y_pred_cat_npy,
                                     pos_label=common.NO_CYCLONE_LABEL)
...
def test(channels_array, chunk_list_df, index_list, netcdf_dict, day, hour,
    ...
    record = chunk_list_df[(chunk_list_df.lat_min == debug_lat_min) & (chunk_list_df.lat_max == debug_lat_max) &
                           (chunk_list_df.lon_min == debug_lon_min) & (chunk_list_df.lon_max == debug_lon_max)]
    print(record)
